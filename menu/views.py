...
class OrderView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        order, _ = Order.objects.get_or_create(
            table_id=table.id, status="P", chef_id=chef.id
        )
...
class OrderModifyView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if kwargs["action"] == "add":
            ...
            try:
                item_in_order = order.orderitem_set.all().get(
                    item__id=kwargs["pk"]
                )
                item_in_order.quantity += 1
                item_in_order.save()
            except OrderItem.DoesNotExist:
                item_in_order = order.orderitem_set.create(
                    item_id=kwargs["pk"], quantity=1
                )
                item_in_order.save()
